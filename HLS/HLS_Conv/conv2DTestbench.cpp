...
///////////////////////////////////////////////////////////////////////////////
void InitVectors(TFXP * input, uint32_t sizeInput, TFXP * coeffs, uint32_t sizeCoeffs, TFXP * biases, uint32_t numBiases)
{
  for (uint32_t ii = 0; ii < sizeInput; ++ ii)
    input[ii] = (rand() % 2) ? rand() : -rand();
  for (uint32_t ii = 0; ii < sizeCoeffs; ++ ii)
    coeffs[ii] = (rand() % 2) ? rand() : -rand();
  for (uint32_t ii = 0; ii < numBiases; ++ ii)
    biases[ii] = (rand() % 2) ? rand() : -rand();
}
...
int main(int argc, char ** argv)
{
  ...
  for (uint32_t iTest = 0; iTest < numSizes; ++ iTest) {
    ...
    printf("  SW\n");
    Conv2D_SW(input, outputSW, coeffs, biases, channels, filters, width, height, 3, 3, 1);
    printf("  HW\n");
    Conv2D_HW(input, outputHW, coeffs, biases, channels, filters, width, height, 3, 3, 1);
