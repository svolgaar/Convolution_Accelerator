      ...
      uint32_t convWidth, uint32_t convHeight, uint32_t relu)
{
  ...
  if (logging)
    printf("CConv2DProxy::Conv2D_HW(Input=0x%08X, Output=0x%08X, Coeffes=0x%08X, biases=0x%08X, "
          "NumChannels=%u, NumFilters=%u, Width=%u, Height=%u, FilterSize=%ux%u, ReLU=%u)\n", 
  ...
  uint32_t phyBiases = GetDMAPhysicalAddr(biases);
  if (phyBiases == 0) {
    if (logging)
      printf("Error: No physical address found for virtual address 0x%08X\n", (uint32_t)biases);
    return VIRT_ADDR_NOT_FOUND;
